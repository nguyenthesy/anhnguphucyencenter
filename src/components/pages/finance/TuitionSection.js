...
import { createPayment, updateStudent } from "@/lib/firestore";
import { formatCurrency, formatDate, getToday } from "@/utils/formatters";

export default function TuitionSection({ students, classes, payments, globalSearch, showToast }) {
  ...
  const filteredStudents = students
    .filter((s) => {
      const matchSearch = !searchTerm || s.fullName.toLowerCase().includes(searchTerm) || (s.className || "").toLowerCase().includes(searchTerm);
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      showToast("Ghi nhận đóng phí thành công!");
    } catch (err) {
      ...
      <div className="grid-2">
          ...
          <div className="card-body" style={{ padding: 0 }}>
            <div className="table-wrapper">
              <table className="table-sm">
                <thead><tr><th>Ngày</th><th>Học viên</th><th>Số tiền</th><th>Ghi chú</th></tr></thead>
                <tbody>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ padding: 0 }}>
            <div className="table-wrapper">
              <table className="table-sm">
                <thead><tr><th>Học viên</th><th>Lớp</th><th>Còn nợ</th></tr></thead>
                <tbody>
