...
export function exportBankStatementPDF({
  ...
  doc.text("TRUNG TAM ANH NGU PHUC YEN", pageWidth / 2, 16, { align: "center" });

  doc.setFontSize(11);
  doc.setFont("helvetica", "normal");
  doc.text("BAO CAO TAI CHINH / SAO KE", pageWidth / 2, 24, { align: "center" });

  doc.setFontSize(9);
  const fromLabel = dateFrom ? formatDateVN(dateFrom) : "---";
  const toLabel = dateTo ? formatDateVN(dateTo) : "---";
  doc.text(`Tu ngay: ${fromLabel}  -  Den ngay: ${toLabel}`, pageWidth / 2, 32, { align: "center" });

  const exportDate = new Date();
  doc.text(
    `Ngay xuat: ${formatDateTimeVN(exportDate)}`,
    pageWidth / 2,
    38,
    { align: "center" }
  );
  ...
  const boxes = [
    { label: "Tong thu", value: formatNum(totalIncome), color: [16, 185, 129] },
    { label: "Tong chi", value: formatNum(totalExpense), color: [239, 68, 68] },
    { label: "So du hien tai", value: formatNum(currentBalance), color: [79, 70, 229] },
  ];
  ...
  autoTable(doc, {
    startY: y,
    head: [["#", "Ngay", "Loai", "Noi dung", "Thu (+)", "Chi (-)", "So du"]],
    body: tableData,
    theme: "grid",
    styles: {
      fontSize: 8,
  ...
  const sigY = finalY + 40 > pageH ? 30 : finalY;
